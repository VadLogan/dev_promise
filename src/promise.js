...
class DevPromise {
      ...
      resRejHandlers(quenuie){
        while(quenuie.length > 0) {
            ...
            if (returnValue instanceof DevPromise) {
                returnValue.then( v =>  resolution.promise.resolve(v))
      ...
      then(resolutionHandler, rejectionHandler) {
          ...
          var newPromise = new DevPromise(function(){});
    ...
    static all(iterable){
        ...
        if(typeof iterable[Symbol.iterator] !== 'function'){
           return new this((res,rej) => rej(TypeError('undefined is not a function')));
        }
        if(!iterable.length){
